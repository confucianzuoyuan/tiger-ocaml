...
tydec:
    | TYPE; x = ID; EQ; t = ty; { TypeDec [{ty_name=(Symbol.symbol x); ty_ty=t}] }
...
fundec:
    | FUNCTION; x = ID; LPAREN; tyfields = separated_list(COMMA, tyfield); RPAREN; EQ; e = exp; { FunctionDec [{fun_name=(Symbol.symbol x); fun_params=tyfields; fun_result=None; fun_body=e}] }
    | FUNCTION; x = ID; LPAREN; tyfields = separated_list(COMMA, tyfield); RPAREN; COLON; y = ID; EQ; e = exp; { FunctionDec [{fun_name=(Symbol.symbol x); fun_params=tyfields; fun_result=Some(Symbol.symbol y); fun_body=e;}] }